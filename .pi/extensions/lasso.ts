...
function safeSetLassoStatus(ctx: LassoStatusContext, text: string | undefined) {
  try {
...
function safeNotify(ctx: LassoStatusContext, message: string, level?: string) {
  try {
...
function updateLassoStatus(ctx: LassoStatusContext) {
  void Promise.all([runLasso(['lint', 'status']), runLasso(['memory', 'status'])])
    ...
    .catch(() => {
      safeSetLassoStatus(ctx, '[lasso: unavailable]');
    });
}
...
function summarizeMemoryStatus(stdout: string) {
  ...
  const entries = stdout.match(/- Entries: (\d+)/)?.[1] ?? '?';
  return `memory ${snapshots} snapshots, ${entries} entries`;
}

function buildMemoryObserverPrompt(conversation: string) {
  return `Extract durable memory from this agent conversation. Return a structured event log with dated, prioritized observations.

Use this format:
## Category Name
- \u{1F534} YYYY-MM-DD: Observation detail [ref:YYYY-MM-DD] [rel:+Nd] (high priority — critical facts, decisions, corrections)
- \u{1F7E1} YYYY-MM-DD: Observation detail (medium priority — useful patterns, preferences)
- \u{1F7E2} YYYY-MM-DD: Observation detail (low priority — minor notes, future reference)

Categories group related observations. Use 2-5 categories. Each bullet gets a priority emoji and date.
Temporal anchors are optional: [ref:YYYY-MM-DD] for dates mentioned in content (e.g. a flight date), [rel:+Nd] or [rel:-Nd] for days relative to the observation date.
Keep only useful project facts, decisions, corrections, and continuation state.
...
function buildAutoReflectionPrompt(conversation: string) {
  return `Create a consolidated memory reflection for this agent session. Summarize durable facts, decisions, patterns, and current state.

Return JSON with keys threadSummary and resourceSummary.
...
async function persistMemoryObservation(ctx: LassoStatusContext, conversation: string, tokens: number) {
  const result = await runPiMemoryModel(ctx, buildMemoryObserverPrompt(conversation));
  const text = result.rawText.trim();
  if (text) await runLasso(['memory', 'observe', '--force', '--tokens', String(tokens)], { input: text });
}
...
async function observeWithLasso(ctx: LassoStatusContext) {
  ...
  // Estimate tokens and check threshold before invoking the LLM observer
  const estimatedTokens = Math.ceil(conversation.length / 4);
  const check = await runLasso(['memory', 'should-observe', '--tokens', String(estimatedTokens)]);
  if (check.exitCode !== 0) {
    // Below threshold, skip observation
    updateLassoStatus(ctx);
    return;
  }

  safeSetLassoStatus(ctx, '[lasso: observing...]');
  void Promise.all([
    runLasso(['lint', 'scan', '--force'], { input: conversation }),
    persistMemoryObservation(ctx, conversation, estimatedTokens),
  ])
    .then(() => triggerReflectionIfNeeded(ctx))
    .catch((error) => {
      safeNotify(ctx, `lasso observation failed: ${error instanceof Error ? error.message : String(error)}`, 'warning');
      updateLassoStatus(ctx);
    });
}
...
async function triggerReflectionIfNeeded(ctx: LassoStatusContext) {
  const check = await runLasso(['memory', 'should-reflect']);
  if (check.exitCode !== 0) {
    updateLassoStatus(ctx);
    return;
  }

  safeSetLassoStatus(ctx, '[lasso: reflecting...]');
  try {
    const conversation = serializePiConversation(ctx);
    const result = await runPiMemoryModel(ctx, buildAutoReflectionPrompt(conversation));
    const summary = typeof result.parsed?.threadSummary === 'string' ? result.parsed.threadSummary : result.rawText;
    if (summary.trim()) await runLasso(['memory', 'reflect'], { input: summary.trim() });
  } catch {
    // Non-critical — reflection will be retried on next threshold breach or compaction
  } finally {
    updateLassoStatus(ctx);
  }
}
...
async function runMirroredCommand(args: string, ctx: LassoStatusContext) {
  ...
  if (commandArgs.length === 0) {
    ...
    safeNotify(ctx, `${lint.stdout}\n${memory.stdout}`.trim(), lint.exitCode || memory.exitCode ? 'error' : 'info');
    return;
  }

  const result = await runLasso(commandArgs);
  safeNotify(ctx, (result.stdout || result.stderr).trim(), result.exitCode === 0 ? 'info' : 'error');
  updateLassoStatus(ctx);
}
...
export default function (pi: ExtensionAPI) {
  ...
  pi.on('before_agent_start', async (event, ctx) => {
    const query = serializePiConversation(ctx).slice(-4000);
    const [memoryResult, workingResult] = await Promise.all([
      runLasso(['memory', 'context', '--query', query]),
      runLasso(['memory', 'working']),
    ]);

    const sections: string[] = [];

    if (memoryResult.exitCode === 0 && memoryResult.stdout.trim() && !memoryResult.stdout.includes('No relevant memory found.')) {
      sections.push(`=== LASSO MEMORY ===\n${memoryResult.stdout.trim()}\n====================`);
    }

    if (workingResult.exitCode === 0 && workingResult.stdout.trim() && !workingResult.stdout.includes('No working memory found')) {
      sections.push(`=== LASSO WORKING MEMORY ===\n${workingResult.stdout.trim()}\n===========================`);
    }
  ...
  pi.on('session_before_compact', async (event, ctx) => {
    ...
    try {
      ...
      safeSetLassoStatus(ctx, undefined);
    }
  });
  ...
  pi.registerCommand('lasso:lint:scan', {
    ...
    handler: async (_args, ctx) => {
      const conversation = serializePiConversation(ctx);
      const result = await runLasso(['lint', 'scan', '--force'], { input: conversation });
      safeNotify(ctx, (result.stdout || result.stderr).trim(), result.exitCode === 0 ? 'info' : 'error');
      updateLassoStatus(ctx);
    },
  });
  ...
  pi.registerCommand('lasso:memory:observe', {
    ...
    handler: async (_args, ctx) => {
      const conversation = serializePiConversation(ctx);
      const tokens = Math.ceil(conversation.length / 4);
      const result = await runLasso(['memory', 'observe', '--force', '--tokens', String(tokens)], { input: conversation });
      safeNotify(ctx, (result.stdout || result.stderr).trim(), result.exitCode === 0 ? 'info' : 'error');
      updateLassoStatus(ctx);
    },
  });
  ...
  pi.registerCommand('lasso:memory:reflect', {
    description: 'Run lasso memory reflect on the current Pi conversation',
    handler: async (_args, ctx) => {
      const conversation = serializePiConversation(ctx);
      const result = await runLasso(['memory', 'reflect'], { input: conversation });
      safeNotify(ctx, (result.stdout || result.stderr).trim(), result.exitCode === 0 ? 'info' : 'error');
      updateLassoStatus(ctx);
    },
  });
