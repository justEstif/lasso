...
export function createEntry(
  ...
  db.prepare(`
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?, ?)
  `).run(id, data.status, data.description, data.proposed_form, data.source_excerpt, data.detector_version, now, now);
...
export function updateEntryStatus(db: Database, id: string, status: LintStatus): void {
  const now = new Date().toISOString();
  db.prepare('UPDATE lint_entries SET status = ?, updated_at = ? WHERE id = ?').run(status, now, id);
}
...
export function addRecurrence(db: Database, entryId: string, note: string): void {
  ...
  db.prepare('UPDATE lint_entries SET updated_at = ? WHERE id = ?').run(new Date().toISOString(), entryId);
}

export function getRecurrences(db: Database, entryId: string): LintRecurrence[] {
  return db.prepare('SELECT * FROM lint_recurrences WHERE entry_id = ? ORDER BY observed_at DESC').all(entryId) as LintRecurrence[];
}
