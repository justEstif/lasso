...
function applyLintMigrations(db: Database) {
  ...
  applyMigration(db, 'lint', 2, () => {
    db.run(`CREATE TABLE IF NOT EXISTS lint_scan_runs (
      id INTEGER PRIMARY KEY AUTOINCREMENT,
...
function applyMigration(db: Database, observer: string, version: number, up: () => void) {
  ...
  if (!isApplied) {
    db.transaction(() => {
      up();
      db.run('INSERT INTO _migrations (observer, version, applied_at) VALUES (?, ?, ?)', [
        observer,
        version,
...
function runStatements(db: Database, statements: string[]) {
